...
// Renderizar productos usando las tarjetas existentes del HTML
function renderProducts() {
    ...
    // Obtener todas las tarjetas de productos del HTML
    const allProductCards = productsGrid.querySelectorAll('.product-card[data-product-id]');
    ...
    // Ocultar mensaje de no encontrados si existe
    const noProductsMessage = productsGrid.querySelector('.no-products');
    if (noProductsMessage) {
        noProductsMessage.style.display = 'none';
    }
    
    // Mostrar/ocultar tarjetas según los productos filtrados
    allProductCards.forEach(card => {
...
function handleSearch() {
    ...
    if (searchTerm === '') {
        ...
        filteredProducts = products.filter(product => {
            ...
            // Búsqueda por tipo de material
            if (product.specifications && product.specifications.Tipo) {
                if (product.specifications.Tipo.toLowerCase().includes(searchLower)) {
                    return true;
                }
            }
            
            // Búsqueda por uso (interior/exterior)
            if (product.specifications && product.specifications.Uso) {
                if (product.specifications.Uso.toLowerCase().includes(searchLower)) {
                    return true;
                }
            }
            
            // Búsqueda por color/acabado
            if (product.specifications && product.specifications.Color) {
                if (product.specifications.Color.toLowerCase().includes(searchLower)) {
                    return true;
                }
            }
            
            if (product.specifications && product.specifications.Acabado) {
                if (product.specifications.Acabado.toLowerCase().includes(searchLower)) {
                    return true;
                }
            }
...
// Función especial para la galería del WPC Interior Negro con alternancia de imágenes
function openWPCInteriorNegroGallery() {
    console.log('Abriendo galería del WPC Interior Negro');
    ...
    // Configurar las imágenes del WPC Interior Negro
    window.wpcInteriorNegroImages = [
        {
            src: 'productos/Wpcinteriornegro.jpg',
            name: 'WPC Interior Negro - Producto',
            description: 'Vista del producto'
        },
        {
            src: 'productos/wpcinteriornegrointegrado.jpg',
            name: 'WPC Interior Negro - Integrado',
            description: 'Producto instalado e integrado'
        }
    ];
    
    window.currentWPCImageIndex = 0;
    
    // Mostrar primera imagen
    const currentImage = window.wpcInteriorNegroImages[0];
    imageZoomImage.src = currentImage.src;
    imageZoomInfo.innerHTML = `
        <strong>${currentImage.name}</strong><br>
        <small>${currentImage.description} (${window.currentWPCImageIndex + 1} / ${window.wpcInteriorNegroImages.length})</small>
    `;
    
    // Configurar navegación para alternar imágenes
    imageZoomNav.innerHTML = `
        <button class="nav-btn" onclick="changeWPCImage(-1)">
            <i class="fas fa-chevron-left"></i>
        </button>
        <button class="nav-btn" onclick="changeWPCImage(1)">
            <i class="fas fa-chevron-right"></i>
...
// Función para cambiar imagen en la galería del WPC Interior Negro
function changeWPCImage(direction) {
    if (!window.wpcInteriorNegroImages) return;
    
    let newIndex = window.currentWPCImageIndex + direction;
    
    if (newIndex < 0) {
        newIndex = window.wpcInteriorNegroImages.length - 1;
    } else if (newIndex >= window.wpcInteriorNegroImages.length) {
        newIndex = 0;
    }
    
    window.currentWPCImageIndex = newIndex;
    const currentImage = window.wpcInteriorNegroImages[newIndex];
    
    const imageZoomImage = document.getElementById('imageZoomImage');
    const imageZoomInfo = document.getElementById('imageZoomInfo');
    
    // Actualizar imagen e información
    imageZoomImage.src = currentImage.src;
    imageZoomInfo.innerHTML = `
        <strong>${currentImage.name}</strong><br>
        <small>${currentImage.description} (${window.currentWPCImageIndex + 1} / ${window.wpcInteriorNegroImages.length})</small>
